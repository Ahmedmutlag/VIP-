...
@app.route("/api/wayl/create", methods=["POST"])
@limiter.limit("10 per minute")
def wayl_create():
    if not WAYL_API_KEY:
        return jsonify({"error": "بوابة الدفع غير متاحة حالياً"}), 503
...
@app.route("/api/wayl/verify", methods=["POST"])
...
def wayl_verify():
    ...
    device_id  = data.get("device_id", "").strip()[:64]
    ...
    try:
        ...
        if resp.status_code == 404:
            return jsonify({"error": "رقم الدفع غير موجود"}), 404
        ...
        with _codes_lock:
            codes = load_codes()
            # Check if this payment_id was already redeemed
            for entry in codes.values():
                if entry.get("wayl_payment_id") == payment_id:
                    return jsonify({"paid": True, "already_redeemed": True,
                                    "expires_at": entry.get("expires_at")})

            code = "WP-" + secrets.token_hex(6).upper()
            expires_at = (now() + timedelta(days=WAYL_PLAN_DAYS)).strftime("%Y-%m-%d %H:%M")
            codes[code] = {
                "used": True,
                "used_at": now().strftime("%Y-%m-%d %H:%M"),
                "expires_at": expires_at,
                "days": WAYL_PLAN_DAYS,
                "wayl_payment_id": payment_id,
                "device_id": device_id,
            }
            save_codes(codes)
